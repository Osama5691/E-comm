...
const Select = ({data,placeholder, icon, view, selectedSelectBoxItem}) => {
    ...
    const filterList=(e)=>{
        ...
        const list = listData2.filter((item)=>{
            return view==="cat" ? item?.name?.toLowerCase().includes(keyword) : item?.toLowerCase().includes(keyword);
        })
                    ...
                    <div className='selectDrop'>
                        ...
                        <li key={0} onClick={() => closeSelect(0, placeholder, '')} className={`${selectedIndex === 0 ? 'active' : ''}`}>{placeholder}</li> 
                        {
                            ...
                            listData.map((item,index)=>{
                                ...
                                return(
                                    <li key={index+1} onClick={() => closeSelect(index+1, view!=="cat" ? item :  item?.name, item?._id)} className={`${selectedIndex === index+1 ? 'active' : ''}`}>
